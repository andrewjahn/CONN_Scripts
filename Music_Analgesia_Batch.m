...
NSUBJECTS=47; % Replace this with the number of subjects that are in the current directory
cwd=pwd;
FUNCTIONAL_FILE=cellstr(conn_dir('sub-*_func_sub-*_task-rest*_bold.nii.gz'));
STRUCTURAL_FILE=cellstr(conn_dir('sub-*_anat_sub-*_T1w.nii.gz'));
...
clear batch;
batch.filename=fullfile(cwd,'Music_Analgesia_Scripted.mat');            % New conn_*.mat experiment name
...
conn_batch(batch)
batch.filename=fullfile(cwd,'Music_Analgesia_Scripted.mat');
...
% clear batch;
% batch.filename=fullfile(cwd,'Music_Analgesia_Scripted.mat');            % Existing conn_*.mat experiment name
...
conn
conn('load',fullfile(cwd,'Music_Analgesia_Scripted.mat'));
conn gui_results
